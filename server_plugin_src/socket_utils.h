...
#include <list>
//---------------------------------------------------------------------------
// Полная информация по физическому соединению.
struct SocketСonnection {
	SocketСonnection(SOCKET _sd, String _ip) : sd(_sd), ip(_ip), in_buffer(NULL), out_buffer(NULL), in_buf_length(0),
...
typedef list<SocketСonnection> SocketСonnections;

extern SOCKET SocketListen(int port);
extern void SocketAccept(SOCKET listening_sock, SocketСonnections &connections, int (*GetSocketMessageLength)(SocketСonnection &, char *, int), bool is_persistent);
